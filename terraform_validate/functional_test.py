...
class TestValidatorFunctional(unittest.TestCase):
    ...
    def test_resource_should_equal_case_insensitive(self):
        ...
        self.assertEqual("[aws_s3_bucket.zzz.value] should be 'aBc'. Is: 'abc'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_resource_not_equals_case_insensitive(self):
        ...
        self.assertEqual("[aws_s3_bucket.zzz.value] should not be 'aBc'. Is: 'abc'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_nested_resource_required_properties_with_list_input(self):
        ...
        self.assertEqual("[aws_instance.foo.nested_resource] should have property: 'abc123'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual("[aws_instance.foo.nested_resource] should have property: 'def456'", self.jsonOutput["failures"][1]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_nested_resource_required_properties_with_string_input(self):
        ...
        self.assertEqual("[aws_instance.foo.nested_resource] should have property: 'def456'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_nested_resource_excluded_properties_with_list_input(self):
        ...
        self.assertEqual("[aws_instance.foo.nested_resource] should not have property: 'value'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual("[aws_instance.foo.nested_resource] should not have property: 'value2'", self.jsonOutput["failures"][1]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_nested_resource_property_value_matches_regex(self):
        ...
        self.assertEqual("[aws_instance.foo.nested_resource.value] should match regex '[a-z]'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_resource_property_invalid_json(self):
        ...
        self.assertEqual("[aws_s3_bucket.invalidjson.policy] is not valid json", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_missing_variable_substitution(self):
        for validator in self.getValidatorGenerator("fixtures/missing_variable"):
            validator.resources('aws_instance').property('value').should_equal(1)
        self.assertEqual("[aws_instance.foo.value] should be '1'. Is: '!{var.missing}'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_searching_for_property_value_using_regex(self):
        ...
        self.assertEqual("[aws_instance.foo.CPM_Service_wibble] should be '2'. Is: '1'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_searching_for_nested_property_value_using_regex(self):
        ...
        self.assertEqual("[aws_instance.foo.tags.CPM_Service_wibble] should be '2'. Is: '1'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_nested_resource_type_list(self):
        ...
        self.assertEqual("[aws_instance.foo.tags.value] should be '2'. Is: '1'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual("[aws_elb.bar.tags.value] should be '2'. Is: '1'", self.jsonOutput["failures"][1]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_invalid_terraform_syntax(self):
        ...
        self.assertEqual(0, len(self.jsonOutput["failures"]), "resource test failed")
        self.assertTrue(self.jsonOutput["errors"][0]["message"].startswith("Traceback"), "resource test failed")
    ...
    def test_multiple_variable_substitutions(self):
        ...
        self.assertEqual("[aws_instance.foo.value] should be '21'. Is: '12'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_nested_multiple_variable_substitutions(self):
        ...
        self.assertEqual("[aws_instance.foo.value_block.value] should be '12'. Is: '21'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_variable_expansion(self):
        ...
        self.assertEqual("[aws_instance.foo.value] should be '${bar.var}'. Is: '1'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_resource_name_matches_regex(self):
        ...
        self.assertEqual("[aws_instance.TEST_RESOURCE] name should match regex: '^[a-z0-9_]*$'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_resource_should_not_exist(self):
        ...
        self.assertEqual("[aws_s3_bucket] should not exist. Found in resource named badResource", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_list_should_not_contain(self):
        ...
        self.assertEqual("[datadog_monitor.bar.tags] '['baz:biz', 'foo:bar']' should not contain '['foo:bar']'.", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_encryption_scenario(self):
        ...
        self.assertEqual("[aws_db_instance_invalid.foo2.storage_encrypted] should be 'True'. Is: 'False'", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual("[aws_db_instance_invalid2.foo3] should have property: 'storage_encrypted'", self.jsonOutput["failures"][1]["message"], "resource test failed")
        self.assertEqual("[aws_instance_invalid.bizz2] should have property: 'encrypted'", self.jsonOutput["failures"][2]["message"], "resource test failed")
        self.assertEqual("[aws_instance_invalid.bizz2.ebs_block_device.encrypted] should be 'True'. Is: 'False'", self.jsonOutput["failures"][3]["message"], "resource test failed")
        self.assertEqual("[aws_instance_invalid2.bizz3] should have property: 'storage_encrypted'", self.jsonOutput["failures"][4]["message"], "resource test failed")
        self.assertEqual("[aws_instance_invalid2.bizz3.ebs_block_device] should have property: 'encrypted'", self.jsonOutput["failures"][5]["message"], "resource test failed")
        self.assertEqual("[aws_ebs_volume_invalid.bar2.encrypted] should be 'True'. Is: 'False'", self.jsonOutput["failures"][6]["message"], "resource test failed")
        self.assertEqual("[aws_ebs_volume_invalid2.bar3] should have property: 'encrypted'", self.jsonOutput["failures"][7]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_with_property(self):
        ...
        self.assertEqual("[aws_s3_bucket.private_bucket.policy] is not valid json", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
    ...
    def test_with_nested_property(self):
        ...
        self.assertEqual("[aws_s3_bucket.tagged_bucket.policy] is not valid json", self.jsonOutput["failures"][0]["message"], "resource test failed")
        self.assertEqual(0, len(self.jsonOutput["errors"]), "resource test failed")
